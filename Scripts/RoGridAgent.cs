...
public class MoveToTargetAgent : Agent
{
    ...
    public static Direction FromAngle(int angle)
    {
        switch (angle)
        {
            ...
            default:
                Debug.LogError($"Unsupported angle: {angle}");
                return Direction.North; // safe fallback
        }
    }
    ...
    public const int k_UpLeft = 5;
    public const int k_UpRight = 6;
    public const int k_DownLeft = 7;
    public const int k_DownRight = 8;
    public const int change_dir_north = 9;
    public const int change_dir_northeast = 10;
    public const int change_dir_east = 11;
    public const int change_dir_southeast = 12;
    public const int change_dir_south = 13;
    public const int change_dir_southwest = 14;
    public const int change_dir_west = 15;
    public const int change_dir_northwest = 16;
    public const int surge = 17;
    public const int powerburst = 18;
    ...
    public List<int> GetActionHistory() { return actionsHistory; }
    ...
    [Header("Debugging")] public AgentActionDebugger debugger;
    ...
    private int GetWeightedRandomFloor(Dictionary<int, float> weights)
    {
        float totalWeight = 0f;
        foreach (var kvp in weights) totalWeight += kvp.Value;
    ...
    {
        // Stage 1.5:
        // int[] problemFloors = new int[] { 40, 25, 5, 40, 40, 40, 9 };
        // int randomIndex = Random.Range(0, problemFloors.Length);
        // int randomStartFloor = problemFloors[randomIndex];

        // Stage 1:
        //int randomStartFloor = Random.Range(1, 50);

        // Stage 1.5:
        //int randomStartFloor = GetWeightedRandomFloor(Constants.floorweights);

        // Stage 2 (& debug):
        int randomStartFloor = 1;

        targetLocation = Constants.floorTargets[randomStartFloor];

        transform.localPosition = Constants.spawnLocations[randomStartFloor];
        agentDirection = FromAngle(Constants.spawnDirections[randomStartFloor - 1]);
        transform.localRotation = Quaternion.Euler(0, Constants.spawnDirections[randomStartFloor - 1], 0);
        dive_cooldown = 0;
        surge_cooldown_1 = 0;
        surge_cooldown_2 = 0;
        ...
        anim_cancel_tick = 0;
        map = agilitymaps.GetMap(randomStartFloor);
        actionsHistory.Clear();
        distanceHistory.Clear();
        agentScore = 0f;
    }

    // Define how far the agent can see (Radius in tiles)
    // Vision of 10 = 21x21 grid = 441 observations
    private const int VISION_RADIUS = 10;

    public override void CollectObservations(VectorSensor sensor)
    {
        // 1. Existing Observations
        Vector3 dirToTarget = (targetLocation - transform.localPosition).normalized;
        sensor.AddObservation(dirToTarget.x);
        sensor.AddObservation(dirToTarget.z);
        sensor.AddObservation(Vector3.Distance(transform.localPosition, targetLocation));

        // 2. The "Virtual" Grid Sensor (Array Lookup)
        int currentX = (int)transform.localPosition.x; // Assumes these are grid-aligned
        int currentZ = (int)transform.localPosition.z; // Assumes these are grid-aligned

        // Map transformation (Matching your isDiveable logic)
        int mapX = currentX + 25;
        int mapZ = (-1 * currentZ) + 25;

        for (int zOffset = -VISION_RADIUS; zOffset <= VISION_RADIUS; zOffset++)
        {
            for (int xOffset = -VISION_RADIUS; xOffset <= VISION_RADIUS; xOffset++)
            {
                int checkX = mapX + xOffset;
                int checkZ = mapZ + zOffset;

                // Check Bounds (Don't crash if looking off the edge of the array)
                if (checkX >= 0 && checkX < MAP_DIAMETER && checkZ >= 0 && checkZ < MAP_DIAMETER)
                {
                    // Add the tile value directly: 0 for floor, 1 for wall
                    // We access the array you already have in memory!
                    int tileValue = map[checkZ, checkX];
                    sensor.AddObservation(tileValue);
                }
                else
                {
                    // If out of bounds, treat as a wall (1)
    ...
    public int isDiveable(int mypos_x, int mypos_z, int targetpos_x, int targetpos_z)
    {
        // 1. SAFETY CHECK: Start AND End positions must be in bounds
        // Previously, you only checked targetpos. If the agent stands on the edge (Index 50), it crashed.
        if (Mathf.Abs(targetpos_x) > 24 || Mathf.Abs(targetpos_z) > 24) return 1;
        if (Mathf.Abs(mypos_x) > 24 || Mathf.Abs(mypos_z) > 24) return 1;
        ...
        // Local function to safely check map bounds
        // Returns "0" (walkable) only if in bounds and actually 0. Otherwise returns 1 (blocked).
        int SafeCheck(int z, int x)
        {
            if (x < 0 || x >= MAP_DIAMETER || z < 0 || z >= MAP_DIAMETER) return 1;
            return map[z, x];
        }
        ...
        while ((vertDist > 0) || (horiDist > 0))
        {
            ...
            // Check Horizontal Move using SafeCheck
            if ((horiDist > 0) && SafeCheck(localpos.First, localpos.Second + horiMag) == 0)
            {
                localpos.Second = localpos.Second + horiMag;
                horiDist = horiDist - 1;
                movedHori = true;
            ...
            // Check Vertical Move using SafeCheck
            if ((vertDist > 0) && SafeCheck(localpos.First + vertMag, localpos.Second) == 0)
            {
                localpos.First = localpos.First + vertMag;
                vertDist = vertDist - 1;
                movedVert = true;
            ...
            // If we haven't reached the goal square but can't move closer
            if (!movedVert && !movedHori)
            {
                return 1;
            }
    ...
    public override void WriteDiscreteActionMask(IDiscreteActionMask actionMask)
    {
        ...
        if (maskActions)
        {
            ...
            int SafeGet(int z, int x)
                {
                    if (x < 0 || x >= MAP_DIAMETER || z < 0 || z >= MAP_DIAMETER) return 1;
                    return map[z, x];
                }

            var upmask = SafeGet(maptranspos_Z - 1, maptranspos_X);
            var downmask = SafeGet(maptranspos_Z + 1, maptranspos_X);
            var leftmask = SafeGet(maptranspos_Z, maptranspos_X - 1);
            var rightmask = SafeGet(maptranspos_Z, maptranspos_X + 1);

            var upleftmask = SafeGet(maptranspos_Z - 1, maptranspos_X - 1);
            var uprightmask = SafeGet(maptranspos_Z - 1, maptranspos_X + 1);
            var downleftmask = SafeGet(maptranspos_Z + 1, maptranspos_X - 1);
            var downrightmask = SafeGet(maptranspos_Z + 1, maptranspos_X + 1);
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        // check stagnation
        if (distanceHistory.Count > 75 &&
            distanceHistory.TakeLast(75).Max() - distanceHistory.TakeLast(75).Min() < 2.0f)
        {
            AddReward(-10.0f);
            EndEpisode();
        }
        ...
        {
            // 1. Calculate Distance After Move
            // Note: 'distanceToTarget' is the distance BEFORE this action (calculated at start of OnActionReceived)
            float newDistance = Vector3.Distance(transform.localPosition, targetLocation);

            // 2. PROGRESS REWARD (Velocity)
            // +1.0 for every unit moved closer, -1.0 for every unit moved away.
            // This perfectly cancels out oscillation (wiggling A->B->A results in 0 net reward).
            // float progress = distanceToTarget - newDistance;
            // AddReward(0.02f * progress);
            AddReward(-0.1f*newDistance);

            // 3. TIME PENALTY
            // The "tax" for existing. This forces the agent to maximize velocity (Surge/Dive)
            // to minimize the total accumulated penalty over the run.
            AddReward(-0.05f);
        ...
        switch (action)
        {
            case k_NoAction:
                ...
                break;
            case k_Right:
                transform.localPosition, transform.localRotation = new Vector3(1f, 0f, 0f);
        ...
        {
            // You don't actually need OnEpisodeEnd() here unless you have custom logic.
            // ML-Agents calls EndEpisode() automatically when max steps are reached
            // IF you set Max Steps in the Inspector.
            // But since you are manual, do this:
            // AddReward(-1.0f); // Optional: Penalty for running out of time
            EndEpisode();
        }
    }
    ...
    private void OnTriggerEnter(Collider other){
        ...
        if (other.TryGetComponent<Target>(out Target target)){
            ...
            {
                if (debugger != null) debugger.CompleteFloor(int.Parse(numberPart));
                AddReward(+1000f);
                ...
                agentDirection = Direction.West; //acts as 0 prior to rotation in the line below?
                transform.localRotation = Quaternion.Euler(0, Constants.spawnDirections[levelnum - 1], 0);
                targetLocation = Constants.floorTargets[levelnum];
